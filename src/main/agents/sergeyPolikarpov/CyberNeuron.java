package main.agents.sergeyPolikarpov;
    ...
    public double[] propagate(double[] inputIn) {
        ...
        for (int m = 0; m < blocksize; m++) {  
        	...
        	for (int n = 0; n < num_bits_in_input; n++) { 
        		if(inputs[m*num_bits_in_input+n] == 1) {tmp_inputs[m] += powstwo[n];}
            }
        }
        ...
        for (int p = 0; p < parallel_inputs; p++) {
         ...
         for (int i = 0; i < sbox[0][0].length; i++) {								//****//
        	 ...
        	 double tmp_value;
        	 if(outputs_for_sbox[i] > this.threshold) {tmp_value = this.threshold;}
        	 else {tmp_value = outputs_for_sbox[i];}
    ...
    public double backPropagate(double[] targetOutputs) {
        ...
        for (int i = 0; i < sbox[0][0].length; i++) {	
        	...
        	if(outputError_tmp == 0) {
        		if(outputError[i] > 0) {outputError[i] = 1;}
        		if(outputError[i] < 0) {outputError[i] = -1;}
        		if(outputError[i] == 0) {outputError[i] = 0;}
        	}
        	else {outputError[i] = outputError_tmp;}  
